...
const level = process.env.LOG_LEVEL ? logLevels[parseInt(process.env.LOG_LEVEL)] : "silent";
// Determine the log file path from the environment variable
const logFilePath = process.env.LOG_FILE || ""; // Empty means no file, logs go to console
const logger = pino({
    level: level,
}, pino.destination({
