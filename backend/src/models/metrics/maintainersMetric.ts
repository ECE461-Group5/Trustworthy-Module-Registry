...
export class MaintainersMetric extends Metric {
  ...
  public async evaluate(card: Scorecard): Promise<void> {
    try {
      ...
      logger.debug(
        `Fetching issues from the last 30 days (since: ${sinceDate.toISOString()})`,
      );
      ...
      card.responsiveMaintainer_Latency = parseFloat(
      ...
      if (averageResponseTime <= 72) {
        ...
        responseScore = 0;
        logger.info(
          "Average response time exceeds 336 hours (14 days). Setting score to 0.",
        );
      }
      ...
      logger.error(
        `Error fetching responsiveness information for ${card.owner}/${card.repo}:`,
        error,
      );
      card.responsiveMaintainer = 0;
    ...
    try {
      ...
      for (const comment of comments) {
        ...
        if (comment.user && validAssociations.includes(comment.author_association)) {
          ...
          const responseTime =
            (commentCreatedAt.getTime() - issueCreatedAt.getTime()) / (1000 * 60 * 60);
          return responseTime;
        }
      }
      ...
      logger.error(
        `Error fetching comments or checking collaborator status for issue #${issue.number}:`,
        error,
      );
      return null;
    }
