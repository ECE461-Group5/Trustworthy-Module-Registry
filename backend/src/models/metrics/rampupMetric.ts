...
export class RampUpMetric extends Metric {
  ...
  public async evaluate(card: Scorecard): Promise<void> {
    try {
      ...
      card.rampUp_Latency = parseFloat(
        ((fetchEndTime - fetchStartTime) / 1000).toFixed(3),
      );
      logger.info(
        `Fetched README for ${card.owner}/${card.repo}, API Latency: ${card.rampUp_Latency} ms`,
      );
      ...
      logger.info(
        `Ramp-up score for ${card.owner}/${card.repo} computed: ${readmeScore}`,
      );
    } catch (error) {
      const err = error as Error;
      logger.error(
        `Error fetching README information for ${card.owner}/${card.repo}: ${err.message}`,
      );
      card.rampUp = 0;
    }
  }
