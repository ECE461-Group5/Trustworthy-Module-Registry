...
(cd ./client && npm run build) && (sudo cp -R build/ /var/www/html/front-end/) && (sudo systemctl reload nginx) >app.out.log 2>app.err.log </dev/null &
# Build FE & BE, start server
(cd ./backend && npm run build && npm run start) && (cd ./client && npm run build) >app.out.log 2>app.err.log </dev/null &
